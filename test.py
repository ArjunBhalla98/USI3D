...
parser.add_argument(
    ...
    "--config",
    type=str,
    default="configs/intrinsic_MIT.yaml",
...
parser.add_argument(
    ...
    "--input_dir",
    type=str,
    default="/home/ros/datasets/intrinsic/MIT-inner-split/trainA",
...
parser.add_argument(
    ...
    "--output_folder",
    type=str,
    default="id_mit_train-inner-opt",
...
parser.add_argument(
    ...
    "--checkpoint",
    type=str,
    default="checkpoints/mit_inner-opt/gen_00440000.pt",
...
with torch.no_grad():
    ...
    for image_name in t_bar:
        ...
        image = Variable(
            transform(Image.open(image_pwd).convert("RGB")).unsqueeze(0).cuda()
        )

        # Start testing
        im_reflect, im_shading = intrinsic_image_decompose(image, wo_fea)
        im_reflect = (im_reflect + 1) / 2.0
        im_shading = (im_shading + 1) / 2.0
